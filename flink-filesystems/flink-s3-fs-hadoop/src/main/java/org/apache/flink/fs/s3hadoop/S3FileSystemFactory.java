...
public class S3FileSystemFactory extends AbstractS3FileSystemFactory {
    ...
    protected URI getInitURI(URI fsUri, org.apache.hadoop.conf.Configuration hadoopConfig) {
        ...
        configuration items dynamically (FLINK-19589). To support this, we can use the 'query'
        parameters allowed in a URI to extract these values and apply them to the Hadoop
        Configuration before passing off to Hadoop-AWS.

        As an example, this can allow Flink connectors to programmatically create an S3 URI
        including an AssumeRoleCredentialProvider and role ARN which will be consumed here. The URI
        will then have the path parameters removed before returning.

        NOTE: Flink maintains a FileSystem cache. In our standalone deployment model, there is no
        risk of buckets with the same name bleeding across accounts, but in a session cluster this
        may well be possible. One workaround would be to update the `FSKey` class to also include
